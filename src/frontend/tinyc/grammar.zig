...
pub const Grammar = struct {
    ...
    pub const Symbol = union(enum) {
        ...
        literal: Lexicon.LiteralKind, // TODO no data here
        identifier,
        nonterm: Nonterm,
        eof, //TODO no eof
    };
    ...
    pub const productions = [_]Production{
        ...
        .{ .id = Id.stmt_list, .left = .stmt_list, .right = &.{ nonTerminal(.statement), nonTerminal(.stmt_list_tail) } },

        // <stmt_list_tail> ::= <statement> <stmt_list_tail>
        //               | ε
        .{ .id = Id.stmt_list_tail_cons, .left = .stmt_list_tail, .right = &.{ nonTerminal(.statement), nonTerminal(.stmt_list_tail) } },
        .{ .id = Id.stmt_list_tail_empty, .left = .stmt_list_tail, .right = &.{} },
